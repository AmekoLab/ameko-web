...
function InactiveBadge({ days }: { days: number }) {
  ...
  return (
    <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold whitespace-nowrap ${colorClass}`}>
      {days} days
    </span>
  );
}
          ...
          <table className="w-full text-left border-collapse cursor-default">
            <thead className="sticky top-0 z-10 bg-neutral-50 text-[11px] text-amazon-textMuted font-medium border-b border-amazon-border shadow-sm">
              <tr>
                ...
                <th className="px-2 py-1.5 font-medium whitespace-nowrap">Inactive</th>
                <th className="px-2 py-1.5 font-medium whitespace-nowrap">Orders</th>
                <th className="px-2 py-1.5 font-medium whitespace-nowrap">Value</th>
                <th className="px-2 py-1.5 font-medium whitespace-nowrap">Action</th>
              </tr>
            </thead>
            <tbody>
              {!data?.items?.length ? (
