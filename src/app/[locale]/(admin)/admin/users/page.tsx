...
export default function AdminUsersPage() {
  ...
  const renderRoleBadge = (role: string) => {
    ...
    switch (role) {
      case "Admin":
        return <span className={`${baseClasses} bg-purple-50 text-purple-700 border-purple-200`}>Admin</span>;
      case "Shop":
        return <span className={`${baseClasses} bg-blue-50 text-blue-700 border-blue-200`}>Shop</span>;
      case "Customer":
        return <span className={`${baseClasses} bg-neutral-50 text-neutral-600 border-neutral-200`}>Customer</span>;
      default:
        return <span className={`${baseClasses} bg-neutral-50 text-neutral-600 border-neutral-200`}>{role}</span>;
    }
  };
  ...
  const renderStatusBadge = (status: number) => {
    ...
    switch (status) {
      case 0:
        return <span className={`${baseClasses} bg-green-50 text-green-700 border-green-200`}>Active</span>;
      case 1:
        return <span className={`${baseClasses} bg-red-50 text-red-700 border-red-200`}>Banned</span>;
      default:
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-2">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <span className="text-[10px] text-amazon-textMuted">
            Total: <strong className="text-amazon-text mx-1">{pagination?.totalCount || 0}</strong> users
          </span>
          <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-neutral-50 border-b border-amazon-border">
                <tr>
                  <th className="px-3 py-2 text-left text-[11px] font-medium text-amazon-textMuted whitespace-nowrap">User</th>
                  <th className="px-3 py-2 text-left text-[11px] font-medium text-amazon-textMuted whitespace-nowrap">Contact</th>
                  <th className="px-3 py-2 text-left text-[11px] font-medium text-amazon-textMuted whitespace-nowrap">Role</th>
                  <th className="px-3 py-2 text-left text-[11px] font-medium text-amazon-textMuted whitespace-nowrap">Email Status</th>
                  <th className="px-3 py-2 text-left text-[11px] font-medium text-amazon-textMuted whitespace-nowrap">Status</th>
                  <th className="px-3 py-2 text-left text-[11px] font-medium text-amazon-textMuted whitespace-nowrap">Created At</th>
                  <th className="px-3 py-2 text-center text-[11px] font-medium text-amazon-textMuted whitespace-nowrap">Actions</th>
                </tr>
              </thead>
              <tbody>
                {userList.map((user) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-sm relative overflow-hidden shrink-0 flex items-center justify-center">
                          {user.image ? (
                            ...
                            <div className="w-6 h-6 rounded-sm bg-neutral-100 border border-amazon-border flex items-center justify-center text-[10px] text-amazon-textMuted font-bold">U</div>
                          )}
                        </div>
                        <div>
                    ...
                    {/* Col 3: Role */}
                    <td className="px-3 py-2 text-[11px] text-amazon-text">{renderRoleBadge(user.roleName)}</td>
                    ...
                    {/* Col 5: Status */}
                    <td className="px-3 py-2 text-[11px] text-amazon-text">{renderStatusBadge(user.status)}</td>
                ...
                {userList.length === 0 && !loading && (
                  <tr>
                    <td colSpan={7} className="px-5 py-12 text-center text-amazon-textMuted text-[11px]">
                      No users found.
                    </td>
                  </tr>
                )}
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-2 border-t border-amazon-border bg-white">
            <p className="text-[10px] text-amazon-textMuted">
              Page <strong className="font-medium text-amazon-text px-0.5">{pagination.currentPage} / {pagination.totalPages}</strong> — {pagination.totalCount} users total
            </p>
            <div className="flex items-center gap-2">
              <button
