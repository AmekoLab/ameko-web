...
const getTransactionDisplay = (tx: WalletTransactionResponse) => {
  ...
  if (tx.flowDirection === "Held") {
    if (tx.type === "SalesPending")
      return { sign: "+", color: "text-orange-500" };
    if (tx.type === "RefundDeduction")
  ...
  return {
    sign: tx.amount > 0 ? "+" : "",
    color: tx.amount > 0 ? "text-green-600" : "text-red-600",
  };
};

export default function ShopWalletDashboard() {
  const t = useTranslations("Financial");
  const [statement, setStatement] = useState<WalletStatementResponse | null>(
    null,
  );
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm">
              <p className="text-sm font-medium text-neutral-500 mb-1 flex items-center gap-1.5">
                <TrendingUp className="w-4 h-4 text-green-500" /> {t("revenue")}
              </p>
              <h3 className="text-2xl font-bold text-green-600">
                {formatCurrency(statement.totalSalesRevenue)}
              </h3>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm">
              <p className="text-sm font-medium text-neutral-500 mb-1 flex items-center gap-1.5">
                <Lock className="w-4 h-4 text-orange-500" /> {t("heldBalance")}
              </p>
              <h3 className="text-2xl font-bold text-orange-600">
                {formatCurrency(statement.closingHeldBalance)}
              </h3>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm">
              <p className="text-sm font-medium text-neutral-500 mb-1 flex items-center gap-1.5">
              ...
              <h3 className="text-2xl font-bold text-red-600">
                -{formatCurrency(statement.totalPlatformFees)}
              </h3>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm">
              <p className="text-sm font-medium text-neutral-500 mb-1 flex items-center gap-1.5">
              ...
              <h3 className="text-2xl font-bold text-red-600">
                -{formatCurrency(statement.totalRefundsDeducted)}
              </h3>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm">
              <p className="text-sm font-medium text-neutral-500 mb-1 flex items-center gap-1.5">
              ...
              <div className="text-xs text-neutral-500 mt-1">
                ...
                {formatCurrency(statement.totalPendingWithdrawals)}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-neutral-200 bg-neutral-50">
              <h3 className="text-lg font-bold text-neutral-900">
                {t("transactionHistory")} ({statement.transactionCount})
              </h3>
            </div>
